...
void jpolld_conn_node(int type, xmlnode x, void *arg)
{
	...
	debug(ERR_DL,"Hi User %s: Infront\nXML: %s\n",
    ...
    case XSTREAM_NODE:
      gettimeofday(&curtime,NULL);
	    debug(ERR_DL,"Hi User %s: Robel Server in.\nXML: %s\n",
		 c->user->id.user,xmlnode2str(x));
	  xmlnode query=xmlnode_get_tag(x,"starttls");
	  if(query)
	  {
		 char* xmlns=xmlnode_get_attrib(query,"xmlns");
      	 if (!j_strcmp(xmlns,"urn:ietf:params:xml:ns:xmpp-tls"))
		 {
		 	c->user->tlsconversionneeded = 1;
		 	debug(ERR_DL,"Match found for TLS start\n");

        
			//t->conns[loc]->ssl = jab_ssl_conn(t->pfds[loc].fd);
			//jab_ssl_connect(t->conns[loc]->ssl);
			//c->user->ssl = 1;
		 }
		
	  }
	  char* pquery = xmlnode_get_name(x);
	  debug(ERR_DL,"Parsed value from %s is %s\n", xmlnode2str(x), pquery );
	  if(strcmp(pquery, "proceed") == 0)
	  {
		 debug(ERR_DL,"Proceeding for TLS\n");
		
	 	c->user->tlsconversionneeded=1;
		t->conns[loc]->ssl = jab_ssl_conn(t->pfds[loc].fd);
		if(t->conns[loc]->ssl == NULL)
			debug(ERR_DL,"TLS is null\n");
		int ip = jab_ssl_connect(t->conns[loc]->ssl);
		debug(ERR_DL,"TLS connect return : %d\n", ip);
		c->user->ssl = 1;
      ...
      break;
    case XSTREAM_ROOT:
      debug(ERR_DL,"User %s: Odpowiedz od serwera.\nXML: %s\n",
		 c->user->id.user,xmlnode2str(x));
      ...
      break;
    case XSTREAM_CLOSE:
	   debug(ERR_DL,"Somthing stream close\n");
