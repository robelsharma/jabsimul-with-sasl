...
void handle_POLLIN(jpolld_thread t, int j, struct timeval* curtime) {
  ...
  debug(ERR_DL,"Poll Read is called\n");
  char line[1024];
  int n;
  int blocked = 0;
  int maxlen=1023;
  if (t->conns[j]->sstate==sock_CONN_IN_PROGRESS) {
    //Jesli przyszly jakies dane, to znaczy, ze nie blokujacy connect powiodl sie
    t->conns[j]->sstate=sock_CONNECTED;
    debug(ERR_DL,"Asynchronous connection OK\n");
  ...
  debug(ERR_DL,"[%li:%li] some event (IN) on fd=%i\n",    /* sielim PATCH */
	curtime->tv_sec,curtime->tv_usec,
	t->pfds[j].fd);
  ...
  if (t->conns[j]->ssl)
  {
	...
	debug(ERR_DL,"TLS Read: %s : %d\n", line, n);
	//user_do_your_job(t->conns[j]->user, &curtime);
  }
  else
  ...
  if( (n <= 0) && (!blocked) ) {                     
	...
	debug(ERR_DL,"TLS Connection closed\n");
  } else {
    SpUs_logreaduser(t->conns[j],"*** %i bytes read:\n%s\n",n,line);
	debug(ERR_DL,"*** %i bytes read:\n%s\n",n,line);
    int iReturn = xstream_eat(t->conns[j]->xs, line, n);
	debug(ERR_DL,"xmpp parse return : %d\n", iReturn);
  ...
  while(1) {
    ...
    if (ut(curtime)-users_check_timestamp > USERS_CHECK_EVENTS_INTERVAL*1000) {
      ...
      for (user=alive_users_list;user;user=user->next) {
	   //debug(ERR_DL,"[ROBEL]:: Here I am user: %s .. this is me\n", user->id.user);
