...
char stream_tls_start[] =
"<?xml version='1.0' ?><stream:stream to='%s' xmlns='jabber:client' xmlns:stream='http://etherx.jabber.org/streams' xml:lang='en' version='1.0'>";
char stream_close[] =
"</stream:stream>";
char login_plain[] =
...
char sasl_plain_login[] = 
"<auth xmlns='urn:ietf:params:xml:ns:xmpp-sasl' mechanism='PLAIN'>%s</auth>";
char sasl_bind_session[] = 
"<iq id=\"%s\" type=\"set\" from='%s@%s' xmlns='jabber:client'><bind xmlns='urn:ietf:params:xml:ns:xmpp-bind'><resource>eye</resource></bind></iq>";
char sasl_presence[] =
"<presence/>";
...
void user_do_your_job(user_properities user, struct timeval* curtime) {
 ...
 // debug(ERR_DL,"Outside User %s: User Do your job when ur state: %d\n",
		// user->id.user, user->state);
  switch (user->state) {
  case UP_DISABLED:
	debug(ERR_DL,"Outside User %s: User Do your job disabled\n",
		 user->id.user);
  return;   //disabled
  case UP_CAN_LOGIN: 
    if (user->conn && user->stream_opened) { 
	  ...
	  //debug(ERR_DL,"User %s: User Do your job\n",
		// user->id.user);
      if (user->digest_auth) user_do_login_digest(user,curtime);
      	else if(user->conn && user->tlsconversionneeded == 1)
		{
			debug(ERR_DL,"[ROBEL]:: Please convert your socket to TLS\n");
			user_accept_tls(user, curtime);
		}
		else if(user->conn && user->tlsconversionneeded == 2)
		{
			debug(ERR_DL,"[ROBEL]:: Socket is converting to TLS after getting proceed\n");
			user_start_sasl_plain_login(user, curtime);
		}
		else if(user->conn && user->tlsconversionneeded == 3)
		{
			debug(ERR_DL,"[ROBEL]:: Stream connection from SASL\n");
			user_open_stream_after_sasl(user, curtime);
		}
		else if(user->conn && user->tlsconversionneeded == 4)
		{
			debug(ERR_DL,"[ROBEL]:: After SASL stream connection open\n");
			user_bind_session_after_sasl(user, curtime);
		}
		else if(user->conn && user->tlsconversionneeded == 5)
		{
			debug(ERR_DL,"[ROBEL]:: Binding complete\n");
			user_do_presence_sasl(user, curtime);
		}
		else if(user->conn && user->tlsconversionneeded == 6)
		{
			debug(ERR_DL,"[ROBEL]:: Presence complete\n");
			//user_do_presence_sasl(user, curtime);
		}
		else
		{
			debug(ERR_DL,"[ROBEL]:: Trying for login when state : %d\r\n", user->state);
			user_do_login_plain(user, curtime);	
		}
    }
    break;
  case UP_IS_LOGGING: 
...
void handle_user_node(xmlnode x, user_properities user, struct timeval* curtime) {
  ...
  debug(ERR_DL,"User %s: handle_user_node\nXML: %s\n",
		 user->id.user,xmlnode2str(x));
  ...
  if (id) {
	...
	else if(j_strcmp(xmlnode_get_name(x),"proceed")==0)
	{
	 user_start_xmpp_tls(user, curtime);
	} 
...
int user_do_login_plain(user_properities user, struct timeval* curtime) {
  ...
  debug(ERR_DL,"User %s: Trying for Plain login with : %s\r\n",user->id.user, xml_buf);
  return 0;
}

int user_accept_tls(user_properities user, struct timeval* curtime)
{
	char xml_buf[] = "<starttls xmlns='urn:ietf:params:xml:ns:xmpp-tls'/>";
	// snprintf(xml_buf,sizeof(xml_buf),login_plain,id,user->id.user,user->passwd,user->id.resource);
	jpolld_conn_write_str(user->conn, xml_buf, 1);
	user->state=UP_CAN_LOGIN;//UP_CAN_LOGIN;//UP_IS_LOGGING;
	user->tlsconversionneeded = 2;
	return 0;
}

int user_start_xmpp_tls(user_properities user, struct timeval* curtime)
{
	char xml_buf[1000]; //= "<starttls xmlns='urn:ietf:params:xml:ns:xmpp-tls'/>";
	snprintf(xml_buf,sizeof(xml_buf),stream_tls_start,user->id.server);
	jpolld_conn_write_str(user->conn, xml_buf, 1);
	user->state=UP_CAN_LOGIN;
	user->tlsconversionneeded = 1;
	return 0;
}

int user_start_sasl_plain_login(user_properities user, struct timeval* curtime)
{

	char xml_buf[20000];
	unsigned char xml_temp[2096];
	int username_len = strlen(user->id.user);
	int password_len = strlen(user->passwd);

	memset(xml_temp, '\0', username_len + password_len + 2);

	memcpy(xml_temp, user->id.user, username_len);
	memcpy(xml_temp + username_len + 1, user->passwd, password_len);
	//debug(ERR_DL,"User %s: Username:Password: %s\r\n",xml_temp);

	int *lenoutput = (int *)malloc(1);
	unsigned char *base64_output = (unsigned char *)malloc(1000);
	base64_output = do_base64_encode(xml_temp, username_len + password_len + 2, lenoutput);
	
  	snprintf(xml_buf,sizeof(xml_buf),sasl_plain_login, base64_output);
  	jpolld_conn_write_str(user->conn, xml_buf, 1);
  	//user_store_xml_info(user,user->id.user,handle_login_response,curtime);	    
 	user->state=UP_CAN_LOGIN;
	user->tlsconversionneeded = 3;
  	debug(ERR_DL,"User %s: Trying for SASL Plain login with : %s\r\n",user->id.user, xml_buf);
...
int user_open_stream_after_sasl(user_properities user, struct timeval* curtime)
{
	char xml_buf[1000];
	snprintf(xml_buf,sizeof(xml_buf),stream_tls_start,user->id.server);
	jpolld_conn_write_str(user->conn, xml_buf, 1);
	user->state=UP_CAN_LOGIN;
	user->tlsconversionneeded = 4;

	debug(ERR_DL,"User %s: Stream for SASL : %s\r\n",user->id.user, xml_buf);
	return 0;
}

int user_bind_session_after_sasl(user_properities user, struct timeval* curtime)
{
	char xml_buf[1000];
	char id[10];
    makeJCOM_ID(user,id,sizeof(id));
  
	snprintf(xml_buf,sizeof(xml_buf),sasl_bind_session, id, user->id.user, user->id.server);
	jpolld_conn_write_str(user->conn, xml_buf, 1);
	user->state=UP_CAN_LOGIN;
	user->tlsconversionneeded = 5;
	user_store_xml_info(user,id,handle_login_response,curtime);	    
  	//user->state=UP_IS_LOGGING;
	debug(ERR_DL,"User %s: Bind Stream for SASL : %s\r\n",user->id.user, xml_buf);
	return 0;
}

int user_do_presence_sasl(user_properities user, struct timeval* curtime)
{
	char xml_buf[1000];
	snprintf(xml_buf,sizeof(xml_buf),sasl_presence);
	jpolld_conn_write_str(user->conn, xml_buf, 1);
	user->state=UP_LOGGED_IN;//UP_IS_LOGGING;
	user->tlsconversionneeded = 6;

	debug(ERR_DL,"User %s: presence for SASL : %s\r\n",user->id.user, xml_buf);
	return 0;
}
